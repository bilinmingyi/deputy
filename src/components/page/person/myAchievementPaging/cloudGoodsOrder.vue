      ...
      <table class="d-table" data-type="2">
        <tbody>
          <tr>
            ...
            <th>
              <div>{{totalAchievement | parseNum | priceFormat}}</div>
            </th>
          </tr>
          <tr>
            <td class="bg-medium">中药饮片</td>
            <td>{{achievementData['1'] | parseNum | priceFormat}}</td>
          </tr>
          <tr>
            <td class="bg-medium">颗粒饮片</td>
            <td>{{achievementData['2'] | parseNum | priceFormat}}</td>
          </tr>
          <tr>
            <td class="bg-medium">中西成药</td>
            <td>{{achievementData['3'] + achievementData['4'] | parseNum | priceFormat}}</td>
          </tr>
          <tr>
            <td class="bg-medium">中药产品</td>
            <td>{{achievementData['5'] | parseNum | priceFormat}}</td>
          </tr>
        </tbody>
      </table>
    </div>
    <div class="bg-fff">
      <div class="achi-item" v-for="item in dataList">
        <div class="achi-item-title">
          <span>{{item.name}}</span>
          <span>
            业绩
            <span class="color-ec6464">{{item.total_price | parseNum | priceFormat}}</span>
          </span>
        </div>
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.axios
        .post("/statistic/goods/personalStatList", params)
        .then(response => {
          let res = response.data;
          if (res.code == 1000) {
            this.achievementData = res.data;
            this.formatDataList(res.data.result);
          } else {
    ...
    formatDataList(data) {
      ...
      for (let clinic in result) {
        ...
        result[clinic].forEach(item => {
          obj[dict[item.type]] += item.price;
          obj.total_price += item.price;
        });
        dataList.push(obj);
      }
