        ...
        <li class="clinic-item" v-for="item in dataList">
          <div class="clinic-item-content">
            ...
            <div>
              ...
              <br>
              <span class="color-8c">待付款：
                <span class="red_a">{{item.sum}}</span>
              </span>
            </div>
          </div>
          <router-link
            :to="{path:`/cloudListPage/clinicCloudOrderPage/${item.clinic_id}`, query:{name:`${item.clinic_name}`}}"
            class="clinic-item-btn"
          >去处理</router-link>
        </li>
...
export default {
  ...
  methods: {
    getData: function(name) {
      ...
      this.showLoad = true;
      self.axios
        .post("/weixin/sales/dyTreatOrder/num", {
          query: name
        })
        .then(respone => {
